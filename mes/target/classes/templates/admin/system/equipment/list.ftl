...
<script type="text/html" id="js-record-table-toolbar-top">
    <div class="layui-btn-container">
        <button class="layui-btn layui-btn-sm" lay-event="add"><i class="layui-icon">&#xe61f;</i>添加设备</button>
    </div>
</script>
...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/admin/sys/equipment/page',
            cols: [
                [{
                    type: 'checkbox'
                }, {
                    field: 'code', title: '设备编码', width: 130
                }, {
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            if (obj.event === 'add') {
                spLayer.open({
                    title: '添加设备',
                    area: ['700px', '500px'],
                    content: '${request.contextPath}/admin/sys/equipment/add-or-update-ui'
                });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/admin/sys/equipment/delete',
                        type: 'POST',
                        data: { id: data.id },
