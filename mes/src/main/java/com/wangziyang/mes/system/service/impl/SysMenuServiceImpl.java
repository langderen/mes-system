...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements ISysMenuService {
     ...
     */
    @Override
    public List<SysMenuDTO> listWithRoleChecked(String roleId) throws Exception {
        return sysMenuMapper.listWithRoleChecked(roleId);
    }

    @Override
    public Map<String, Object> listIndexMenuTreeByRoleIds(Set<String> roleIds) throws Exception {
        List<SysMenu> allowedMenus = new ArrayList<>();
        if (roleIds != null && !roleIds.isEmpty()) {
            allowedMenus = sysMenuMapper.listByRoleIds(new ArrayList<>(roleIds));
        }
        return buildFilteredMenuTree(allowedMenus);
    }

    @Override
    public Map<String, Object> listIndexMenuSearchTreeByRoleIds(String menuName, Set<String> roleIds) throws Exception {
        List<SysMenu> searchMenus = sysMenuMapper.listBySearchByName(menuName);
        List<SysMenu> allowedMenus = new ArrayList<>();
        if (roleIds != null && !roleIds.isEmpty()) {
            List<SysMenu> roleMenus = sysMenuMapper.listByRoleIds(new ArrayList<>(roleIds));
            Set<String> allowedIds = new HashSet<>();
            for (SysMenu m : roleMenus) {
                allowedIds.add(m.getId());
            }
            for (SysMenu m : searchMenus) {
                if (allowedIds.contains(m.getId())) {
                    allowedMenus.add(m);
                }
            }
        }
        return buildSearchFilteredMenuTree(allowedMenus);
    }

    private Map<String, Object> buildFilteredMenuTree(List<SysMenu> allowedMenus) {
        Map<String, Object> result = new LinkedHashMap<>(4);
        ...
        if (allowedMenus != null && !allowedMenus.isEmpty()) {
            ...
            Map<String, SysMenu> allMenuMap = new HashMap<>();
            for (SysMenu m : allMenus) {
                allMenuMap.put(m.getId(), m);
            }

            Set<String> visibleIds = new HashSet<>();
            for (SysMenu m : allowedMenus) {
                visibleIds.add(m.getId());
                String parentId = m.getParentId();
                while (parentId != null && !"0".equals(parentId) && allMenuMap.containsKey(parentId)) {
                    visibleIds.add(parentId);
                    parentId = allMenuMap.get(parentId).getParentId();
                }
            }

            List<TreeVO<SysMenu>> menus = new ArrayList<>();
            for (SysMenu m : allMenus) {
                if (!visibleIds.contains(m.getId())) {
                    continue;
                }
                TreeVO<SysMenu> tree = new TreeVO<>();
                tree.setId(m.getId());
    ...
    private Map<String, Object> buildSearchFilteredMenuTree(List<SysMenu> allowedMenus) {
        Map<String, Object> result = new LinkedHashMap<>(4);
        Map<String, Object> menuInfo = new LinkedHashMap<>(8);
